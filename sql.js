...
const goldMedalNumber = country => {
    return `SELECT COUNT(id) AS count FROM GoldMedal WHERE country="${country}"`;
};
...
const mostMedaledAthlete = country => {
  return `WITH athlete AS (SELECT name, COUNT(*) AS count FROM GoldMedal WHERE country="${country}" GROUP BY name ORDER BY count DESC LIMIT 1) SELECT name FROM athlete`;
};
...
const orderedMedals = (country, field, sortAscending) => {
  if (field) {
    ...
    return `SELECT * FROM GoldMedal WHERE country="${country}" ORDER BY ${field} ${direction}`;
  } else {
...
const orderedSports = (country, field, sortAscending) => {
  if (field) {
    ...
    return `SELECT sport, COUNT(*) AS count, 100*COUNT(*)/(SELECT COUNT(*) FROM GoldMedal WHERE country="${country}") AS percent FROM GoldMedal WHERE country="${country}" GROUP BY sport ORDER BY ${field} ${direction}`;
  } else {
    return `SELECT sport, COUNT(*) AS count, 100*COUNT(*)/(SELECT COUNT(*) FROM GoldMedal WHERE country="${country}") AS percent FROM GoldMedal WHERE country="${country}" GROUP BY sport`;
  }
};
